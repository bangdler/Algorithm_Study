...
function insertionSort(array) {
    for(let i = 1; i < array.length; i++) {
        ...
        // 정렬된 배열의 오른쪽부터 순회. 현재값보다 정렬배열의 값이 크면 정렬배열의 값을 오른쪽으로 바꿔준다.
        let j;
        for(j = i-1; j >= 0 && array[j] > curValue; j--) {
            console.log('위치 이동', array, array[j], "->", array[j+1])
            array[j+1] = array[j] // array[j] 정려배열 값을 오른쪽으로 이동
            console.log('이동 완료', array)
        }
        console.log('마지막 바꾸기', array, array[j+1], "<-", curValue)
        array[j+1] = curValue // 현재값이 들어갈 위치는 array[j] < curValue 인 위치의 다음.
